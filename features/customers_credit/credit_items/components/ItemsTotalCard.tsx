...
function ItemsTotalCard({credit, itemCount}: { credit: ModelCredit, itemCount: number }) {
    ...
    const {paidAmount, totalAmount,id} = credit
    ...
    return (
        <View style={{
            ...
            <View className="flex items-center justify-center flex-row gap-3 ">
                <View className="flex-1">
                    <IconButton onPress={navigatePaymentHistory} icon={<Octicons name="history" size={18} color={isDark ? "#ffffff" : colors.primary}/>}
                                label={"View Payment"} variant={"outline"}
                                containerClassName={"py-4    rounded-lg"}/>
                </View>
                <View className="flex-1">

                    <IconButton onPress={navigateAddPayment} icon={<Wallet color={"white"} size={18} strokeWidth={1.5}/>} label={"Add Payment"}
                                containerClassName={"py-4   rounded-lg"}/>
